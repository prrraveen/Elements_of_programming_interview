...
def online_median(sequence: Iterator[int]) -> List[float]:
    # min_heap stores the larger half seen so for
    min_heap = []
    ...
    for x in sequence:
        heapq.heappush(max_heap, -heapq.heappushpop(min_heap, x))
        # Ensure min_heap and max_heap have equal number of elements if an even numbers of elements is read.
        # Otherwise, min_heap must have one more element than max
        if len(max_heap) > len(min_heap):
            heapq.heappush(min_heap, -heapq.heappop(max_heap))
